...
class AIGateway:
    ...
    def chat(
        ...
        **kwargs,
        ...
        if settings.ENVIRONMENT != "production" or last_error is not None:
            logger.warning(f"AI Gateway provider execution fallback ({last_error}). Returning structured answer.")
            return {
    ...
    def stream(
        ...
        **kwargs,
        ...
        from api.core.config import settings
        if settings.ENVIRONMENT != "production" or last_error is not None:
            logger.warning(f"AI Gateway streaming execution fallback ({last_error}). Yielding structured tokens stream.")
            yield {
