...
class OpenAIProvider(BaseLLMProvider):
    ...
    def vision(
        self, prompt: str, image_url: str, model: str = "gpt-4o-mini"
    ) -> Dict[str, Any]:
        messages = [
            {
                "role": "user",
        ...
        return self.chat(messages=messages, model=model or "gpt-4o-mini")
    ...
    async def acheck_connectivity(self, timeout: float = 10.0) -> Dict[str, Any]:
        """
        Real async network liveness check: list available OpenAI models.
        """
        api_key = self.api_key or os.getenv("OPENAI_API_KEY")
        if not api_key:
            return {"reachable": False, "latency_ms": 0, "error": "OpenAI API key is not configured."}
    ...
    def check_connectivity(self, timeout: float = 10.0) -> Dict[str, Any]:
        """
        Real network liveness check: list available OpenAI models with configured key.
        """
        api_key = self.api_key or os.getenv("OPENAI_API_KEY")
        if not api_key:
            return {"reachable": False, "latency_ms": 0, "error": "OpenAI API key is not configured."}
