...
class MistralProvider(BaseLLMProvider):
    def __init__(self, api_key: Optional[str] = None, base_url: Optional[str] = None) -> None:
        super().__init__(
            ...
            base_url=(base_url or os.getenv("MISTRAL_BASE_URL") or "https://api.mistral.ai/v1").rstrip("/")
        )
    ...
    async def aembeddings(self, text: str, model: str = "mistral-embed") -> List[float]:
        api_key = self.api_key or os.getenv("MISTRAL_API_KEY")
        if not api_key:
            raise RuntimeError("Mistral API key is not configured.")
    ...
    def embeddings(self, text: str, model: str = "mistral-embed") -> List[float]:
        api_key = self.api_key or os.getenv("MISTRAL_API_KEY")
        if not api_key:
            raise RuntimeError("Mistral API key is not configured.")
    ...
    def vision(
        ...
        raise NotImplementedError("Mistral models do not support multimodal vision inputs.")
    ...
    async def acheck_connectivity(self, timeout: float = 10.0) -> Dict[str, Any]:
        """
        Real async network liveness check against Mistral models API.
        """
        api_key = self.api_key or os.getenv("MISTRAL_API_KEY")
        if not api_key:
            return {"reachable": False, "latency_ms": 0, "error": "Mistral API key is not configured."}
    ...
    def check_connectivity(self, timeout: float = 10.0) -> Dict[str, Any]:
        """
        Real network liveness check against Mistral models API.
        """
        api_key = self.api_key or os.getenv("MISTRAL_API_KEY")
        if not api_key:
            return {"reachable": False, "latency_ms": 0, "error": "Mistral API key is not configured."}
