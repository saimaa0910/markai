...
class AgentRun(Base):
    ...
    evaluation: Mapped[Optional["AgentEvaluation"]] = relationship(
        "AgentEvaluation", back_populates="run", cascade="all, delete-orphan", uselist=False
    )
...
class AgentEvaluation(Base):
    """
    Persisted evaluation scores for a single AgentRun.
    Generated by the Reflection + Evaluation pipeline after each run.
    Scores are in the range 0.0 - 1.0.
    """
    __tablename__ = "agent_evaluations"
    __table_args__ = (
        Index("ix_agent_evaluations_run_id", "run_id"),
        Index("ix_agent_evaluations_org_id", "organization_id"),
    )
    ...
    tool_usage_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    knowledge_usage_score: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    suggested_edits: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    is_satisfactory: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)

    # Raw evaluation metadata
    meta_data: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)

    # Relationships
    run: Mapped["AgentRun"] = relationship("AgentRun", back_populates="evaluation")
