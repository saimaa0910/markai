...
export default function AgentPlaygroundPage() {
  ...
  // Handle auto-session creation/lookup
  React.useEffect(() => {
    if (selectedAgentId) {
  ...
  }, [selectedAgentId, sessions, createSession]);

  // Sync historical runs into messages
  React.useEffect(() => {
    if (runs.length > 0) {
      const msgs: Message[] = [];
